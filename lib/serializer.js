...
internals.deserializeAttribute = function (value, key, datatypes, callback) {
  ...
  } else if (value && key && datatypes && datatypes[key] && datatypes[key].indexOf('ENCRYPTED:') === 0 && internals.kms) {
...
serializer.serializeItem = function (schema, item, options, callback) {
  ...
  const serializeAsync = function(item, datatypes, callback) {
    ...
    async.eachOfSeries(item, async.ensureAsync(pairHandler), function(error) {
      if (error) {
        callback(error);
      } else {
...
serializer.deserializeItem = function (item, schema, callback) {
  ...
  const formatter = function (data, key, callback) {
    ...
    const processArrayItem = function(value, cb) {
      ...
      if(_.isPlainObject(value) || _.isArray(value)) {
        ...
        internals.deserializeAttribute(value, key, schema ? schema._modelDatatypes : void(0), function(error, data) {
          if (error) {
            cb(error);
          } else {
            mapped.push(data);
    ...
    const processArrayItems = function(cb) {
      if (_.isArray(data)) {
        ...
        async.eachSeries(data, async.ensureAsync(processArrayItem), function(error) {
          if (error) {
            cb(error);
          } else {
            cb(NO_ERROR, DONE);
    ...
    const processMapItems = function(cb) {
      if (!_.isArray(data)) {
        mapped = {};
        async.mapValuesSeries(data, async.ensureAsync(processMapItem), function(error) {
          if (error) {
            cb(error);
          } else {
            cb(NO_ERROR, DONE);
