...
vogels.encryptionPlugin = internals.encryptionPlugin = function(plugin) {
  internals.encryptionPlugin = plugin;
  serializer.setEncryptionPlugin(plugin);
};
// vogels.kmsDriver = internals.kmsDriver = function(kms) {
//   if (kms) {
//     internals.kms = kms;
//     serializer.setKMS(kms);
//   }
// };

internals.updateKMSForAllModels = function (kms) {
  _.each(vogels.models, function (model) {
    model.config({kms: kms});
  });
};
