...
const Scan = module.exports = function (table, serializer) {
  ...
  this.options = {loadAll: false, SkipDecrypt: false};
...
Scan.prototype.attributes = function(attrs) {
  ...
  const expressionAttributeNames = _.reduce(attrs, function (result, attr) {
    const path = '#' + attr;
    result[path] = attr;
...
Scan.prototype.skipDecrypt = function() {
  this.options.SkipDecrypt = true;
  return this;
};
...
Scan.prototype.exec = function(callback) {
  ...
  const runScan = function (params, callback) {
    const options = {
      SkipDecrypt : self.options.SkipDecrypt
    };
    self.table.runScan(params, options, callback);
  };
